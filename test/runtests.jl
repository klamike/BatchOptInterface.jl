...
function create_test_models()
    models = ExaModel[]
    push!(models, create_luksan_vlcek_model(5; M = 1))
    push!(models, create_luksan_vlcek_model(8; M = 1))
    push!(models, create_luksan_vlcek_parametric_model(5; M = 1))
    return models
end
...
@testset "ExaModels Loop" begin
    models = create_test_models()

    model_names = ["Luksan-Vlcek 5", "Luksan-Vlcek 8", "Parametric Luksan-Vlcek 5"]
    
    for (i, model) in enumerate(models)
        @testset "$(model_names[i]) Model" begin
